...
public class ZertifikatRepo {
	...
	int minimumWaittimeMinutes;

	@ConfigProperty(name = "vacme.zertifikat.minimumwait.min.revocation.post.hours", defaultValue = "48")
	int minimumWaittimeRevocationPostHours;

	private final Db db;
	private ObjectMapper mapper;

	@Inject
	public ZertifikatRepo(Db db, ObjectMapper mapper) {
		this.db = db;
		this.mapper = mapper;
	}
	...
	public List<ZertifikatCreationDTO> findImpfungenForZertifikatsGeneration(List<RegistrierungsEingang> eingangs, long covidapiBatchSize) {
		LocalDateTime minimumWaitTime = LocalDateTime.now().minusMinutes(minimumWaittimeMinutes);
	...
	public List<ZertifikatCreationDTO> findAllMigrationImpfungenForZertifikatsGenerationPostRegexbased(String migrationRegex, long availableMigrationBatchSize) {
		...
		String queryImpfung1 =
			"select R.registrierungsnummer, I.id as impfungUUID "
				...
				+ "ORDER BY R.timestampZuletztAbgeschlossen ASC "
				+ "LIMIT ?4 ;";
	...
	private List<ZertifikatCreationDTO> baseImpfungen1ForZertifikatsGeneration(
		...
		return db.select(Projections.constructor(ZertifikatCreationDTO.class, registrierung.registrierungsnummer, QImpfung.impfung.id))
			.from(QImpfung.impfung)
			.innerJoin(QImpftermin.impftermin).on(QImpftermin.impftermin.eq(QImpfung.impfung.termin))
			.innerJoin(QRegistrierung.registrierung).on(QRegistrierung.registrierung.impftermin1.eq(QImpftermin.impftermin))
			.distinct()
			.where(predicateImpfungenForZertifikatGeneration(eingangstypen, minimumWaitTime))
			.orderBy(registrierung.timestampZuletztAbgeschlossen.asc()) // Die aeltesten zuerst
	...
	private List<ZertifikatCreationDTO> baseImpfungen2ForZertifikatsGeneration(
		...
		return db.select(Projections.constructor(ZertifikatCreationDTO.class, registrierung.registrierungsnummer, QImpfung.impfung.id))
			.from(QImpfung.impfung)
			.innerJoin(QImpftermin.impftermin).on(QImpftermin.impftermin.eq(QImpfung.impfung.termin))
			.innerJoin(QRegistrierung.registrierung).on(QRegistrierung.registrierung.impftermin2.eq(QImpftermin.impftermin))
			.distinct()
			.where(predicateImpfungenForZertifikatGeneration(eingangstypen, minimumWaitTime))
			.orderBy(registrierung.timestampZuletztAbgeschlossen.asc()) // Die aeltesten zuerst
			.limit(covidapiBatchSize)
	...
	public List<UUID> findZertifikateForZertifikatsRevocationPost(long batchsize) {
		return findZertifikateForZertifikatsRevocation(batchsize, minimumWaittimeRevocationPostHours, ZertifikatQueueTyp.REVOCATION_POST);
	}
