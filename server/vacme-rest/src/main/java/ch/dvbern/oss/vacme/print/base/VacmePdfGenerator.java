...
public abstract class VacmePdfGenerator {
	...
	private void initGenerator() {
		BaseLayoutConfiguration layoutConfiguration = MandantFactory.getLayoutConfiguration(getAbsenderBlock());
	...
	public void addBarcode(@NonNull Document document) {
		try {
			final Config config = ConfigProvider.getConfig();
			String baseUrl = config.getValue("vacme.server.base.url", String.class);
			String url = baseUrl + "/dossier/" + registrierung.getRegistrierungsnummer();
			...
			PdfPCell cell1 =
				new PdfPCell(new Phrase(translateAllConfiguredLanguages("qrcode_title", "\n"), PdfConstants.DEFAULT_FONT));
			...
			// Ihre Prioritaet
			table.addCell(createCell(new Phrase(translateAllConfiguredLanguages("qrcode_priorisierung", " / "),
				PdfConstants.DEFAULT_FONT)));

			// Prio-Code
			table.addCell(createCell(new Phrase(getPrioritaetText(registrierung.getPrioritaet()), PdfConstants.FONT_TITLE)));
	...
	protected void doForAllLanguagesSeparated(@NonNull BlockPerLanguage blockPerLanguage, @NonNull Runnable separatorBlock) {
