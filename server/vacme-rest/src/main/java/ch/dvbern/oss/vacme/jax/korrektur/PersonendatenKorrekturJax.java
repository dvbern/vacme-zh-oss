...
public class PersonendatenKorrekturJax {
	...
	public Consumer<Fragebogen> getUpdateEntityConsumer() {
		return fragebogen -> {
			...
			personenkontrolle.setIdentifikationsnummer(identifikationsnummer);
			fragebogen.setImmunsupprimiert(immunsupprimiert);
		};
	}

	public boolean needsNewZertifikat(@NonNull Registrierung dataBeforeCorrection) {
		if (dataBeforeCorrection.abgeschlossenMitVollstaendigemImpfschutz()) {
			// Im Zertifikat sind Name, Vorname, Geburtsdatum
			final boolean personalienSame = isPersonalienSame(dataBeforeCorrection);
			// Fuer Online Faelle wird die Adresse nicht beachtet
			boolean adresseSameOrNotRelevant = isAdresseSameOrNotRelevant(dataBeforeCorrection);
	...
	private boolean sameOrBothEmpty(String first, String second) {
