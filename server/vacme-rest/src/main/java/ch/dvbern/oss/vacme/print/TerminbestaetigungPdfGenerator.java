...
public class TerminbestaetigungPdfGenerator extends VacmePdfGenerator {
	...
	public TerminbestaetigungPdfGenerator(@NotNull Registrierung registrierung, @Nullable Impftermin boosterTerminOrNull) {
		super(registrierung);
	...
	protected PdfGenerator.CustomGenerator getCustomGenerator() {
		return (generator) -> {
			...
			if (!registrierung.isNichtVerwalteterOdiSelected()) {
				...
				final Impftermin termin1 = registrierung.getImpftermin1();
				...
				if (ortDerImpfung.isTerminverwaltung()) {
					if (RegistrierungStatus.getAnyStatusOfGrundimmunisiert().contains(registrierung.getRegistrierungStatus())) {
						if (boosterTerminOrNull != null) {
							table.addCell(createLeftCell(new Phrase(translateAllConfiguredLanguages("print_terminbestaetigung_terminN", " / "),
								PdfConstants.DEFAULT_FONT_BOLD)));
							table.addCell(createLeftCell(new Phrase(boosterTerminOrNull.getTerminZeitfensterStartDateAndTimeString() + "\n ",
						...
						if (hasTermin1) {
							...
							if (!RegistrierungStatus.isErsteImpfungDoneAndZweitePending().contains(registrierung.getRegistrierungStatus())) {
								table.addCell(createLeftCell(new Phrase(translateAllConfiguredLanguages("print_terminbestaetigung_termin1", " / "),
									PdfConstants.DEFAULT_FONT_BOLD)));
								table.addCell(createLeftCell(new Phrase(termin1.getTerminZeitfensterStartDateAndTimeString() + "\n ", PdfConstants.DEFAULT_FONT)));
							} else {
								LOG.debug("Generated Terminbestaetigung without pending Termin 1 fuer Reg {}", registrierung.getRegistrierungsnummer());
							}
						} else {
							LOG.error("Trying to generate Terminbestaetigung, but no Termin 1 was found! Registrierung {}",
								registrierung.getRegistrierungsnummer());
						}
						if (hasTermin2) {
							table.addCell(createLeftCell(new Phrase(translateAllConfiguredLanguages("print_terminbestaetigung_termin2", " / "),
								PdfConstants.DEFAULT_FONT_BOLD)));
				...
				if (StringUtils.isNotEmpty(ortDerImpfung.getKommentar())) {
					table.addCell(createLeftCell(new Phrase(translateAllConfiguredLanguages("print_terminbestaetigung_kommentar_odi", " / "),
						PdfConstants.DEFAULT_FONT_BOLD)));
				} else {
			...
			EmptyLinesForLocale emptyLinesAfter = (Locale locale) ->
				locale.equals(Locale.GERMAN) ? ABSTAND :
					(locale.equals(Locale.FRENCH) ? getEmptyLinesAfterFrenchText()
						: 0);

			if (ortDerImpfung != null && ortDerImpfung.isTerminverwaltung()) {
				doForAllLanguagesSeparated(locale -> {
					document.add(PdfUtil.createParagraph(translate("print_terminbestaetigung_anweisung_mit_termine", locale),
						emptyLinesAfter.get(locale), PdfConstants.FONT_RED));
				}, document);
			} else if (ortDerImpfung != null && ortDerImpfung.isMobilerOrtDerImpfung()) {
				doForAllLanguagesSeparated(locale -> {
					document.add(PdfUtil.createParagraph(translate("print_terminbestaetigung_anweisung_mobiler_ort_der_impfung", locale), ABSTAND,
						PdfConstants.FONT_RED));
				}, document);
			} else {
				doForAllLanguagesSeparated(locale -> {
					document.add(PdfUtil.createParagraph(translate("print_terminbestaetigung_anweisung_ohne_termine", locale), ABSTAND,
						PdfConstants.FONT_RED));
				}, document);
			}
		};
	...
	private final String getAdresseString(final Adresse adresse) {
	...
	private OrtDerImpfung getOrtDerImpfung() {
		final boolean isAlreadyGrundimmunisiert =
			RegistrierungStatus.getAnyStatusOfGrundimmunisiert().contains(registrierung.getRegistrierungStatus());
		final boolean isErsteImpfungDurch =
			RegistrierungStatus.isErsteImpfungDoneAndZweitePending().contains(registrierung.getRegistrierungStatus());
		OrtDerImpfung relevantOdi = null;
		if (isAlreadyGrundimmunisiert) {
			if (boosterTerminOrNull != null) {
				relevantOdi = boosterTerminOrNull.getImpfslot().getOrtDerImpfung();
			} else {
				relevantOdi = registrierung.getGewuenschterOdi();
			}
		} else if (isErsteImpfungDurch) {
			if (registrierung.getImpftermin2() != null) {
				relevantOdi = registrierung.getImpftermin2().getImpfslot().getOrtDerImpfung();
			} else {
				relevantOdi = registrierung.getGewuenschterOdi();
			}
		} else {
			if (registrierung.getImpftermin1() != null) {
				relevantOdi = registrierung.getImpftermin1().getImpfslot().getOrtDerImpfung();
			} else {
				relevantOdi = registrierung.getGewuenschterOdi();
			}
		}
