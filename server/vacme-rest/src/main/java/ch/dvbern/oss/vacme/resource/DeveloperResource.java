...
public class DeveloperResource {
	...
	public Response callBoosterRecalculation() {
		try {
			int numCreated = boosterService.queueAllRegsForBoosterRuleengine();
	...
	@POST
	@Path("/impfungen/changeVerantwortlicher")
	@RolesAllowed(BenutzerRolleName.AS_REGISTRATION_OI)
	@Transactional(TxType.NOT_SUPPORTED)
	@Consumes(MediaType.TEXT_PLAIN)
	@Produces(MediaType.TEXT_PLAIN)
	public Response changeVerantwortlicher(@NotNull @NonNull String csvWithImpfids) {
		List<Pair<UUID, UUID>> impfungenAndOdiUUIDs =
			massenmutationService.parseAsImpfungenAndOdiUUIDs(csvWithImpfids);
		List<Pair<ID<Impfung>, ID<Benutzer>>> listWithIds = toImpfungAndBenutzerId(impfungenAndOdiUUIDs);
		massenmutationService.addImpfungenToChangeVerantwortlicherQueue(listWithIds);
	...
	public Response addAllToImpfdossierEnsureQueue() {
		List<String> regsWithoutImpfdossier = impfdossierCreationService.findRegsWithoutImpfdossier();
	...
	@NonNull
	private List<Pair<ID<Impfung>, ID<Benutzer>>> toImpfungAndBenutzerId(
		@NonNull List<Pair<UUID, UUID>> impfungenAndBenutzerUUIDs
	) {
		return impfungenAndBenutzerUUIDs.stream()
			.map(uuiduuidPair -> Pair.of(Impfung.toId(uuiduuidPair.getLeft()),
				Benutzer.toId(uuiduuidPair.getRight())))
			.collect(Collectors.toList());
	}
}
