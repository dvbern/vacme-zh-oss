...
import ch.dvbern.oss.vacme.print.base.MandantFactory;
...
public class ArchivierungPdfGenerator {
	...
	public void createArchivierung(@NonNull OutputStream outputStream) throws IOException {
		createArchivierung(outputStream, null);
	}

	public void createArchivierung(@NonNull OutputStream outputStream, @Nullable List<RegistrierungFile> uploadedFiles) throws IOException {
	...
	private void fillPDF(@Nullable List<RegistrierungFile> uploadedFiles) throws IOException {
		...
		document.add(createKeyValue("Status", getRegistrierung().getRegistrierungStatus().name()));
		...
		if (impfinformationDto.getImpfung1() != null) {
			addImpfung(1, impfinformationDto.getImpfung1(),
				fragebogen.getPersonenkontrolle().getKontrolleTermin1(), document);
		}
		if (impfinformationDto.getImpfung2() != null) {
			addImpfung(2, impfinformationDto.getImpfung2(),
				fragebogen.getPersonenkontrolle().getKontrolleTermin2(), document);
		}
	...
	private void mergeFile(@NonNull List<RegistrierungFile> registrierungFileList,  @NonNull Document document) throws IOException {
		for (RegistrierungFile registrierungFile : registrierungFileList) {
			FileBlob fileBlob = registrierungFile.getFileBlob();
			...
			if (fileBlob.getMimeType().equals(MimeType.APPLICATION_PDF)) {
				...
				com.itextpdf.layout.element.Image image = new com.itextpdf.layout.element.Image(ImageDataFactory.create(registrierungFile.getContent()));
				final BaseLayoutConfiguration layoutConfiguration = MandantFactory.getLayoutConfiguration(Collections.emptyList());
