...
import ch.dvbern.oss.vacme.reports.abrechnung.AbrechnungDTO;
import ch.dvbern.oss.vacme.reports.abrechnung.AbrechnungDataRow;
import ch.dvbern.oss.vacme.reports.abrechnungZH.AbrechnungZHDTO;
...
public class AbrechnungRepo {
	...
	private List<AbrechnungDTO> runOdiAbrechnungRegistrierungsimpfungenQuery(
		...
		QImpfung impfung, BooleanTemplate booleanTemplate, Predicate regJoinExporession, String currentJoinName) {
		StopWatch stopWatch = StopWatch.createStarted();
		...
		List<AbrechnungDTO> result = getAbrechnungRootQuery(dateVon, dateBis, odi, termin, slot, impfung)
			.innerJoin(registrierung).on(regJoinExporession)
	...
	private List<AbrechnungDataRow> readSelbstzahlendeToResult(
		...
		Map<OrtDerImpfung, Long> odiCountSelbstzahlendeZweitImpfung = getSelbstzahlendeRegistrierungsimpfungen(
			dateVon, dateBis, odi, termin, slot, qImpfung, getBooleanExpImpftermin2(), booleanTemplate);
		...
		for (OrtDerImpfung odiSelbstzahlende : odiCountSelbstzahlendeMap.keySet()) {
			...
			AbrechnungDataRow existingRow = result.stream()
				.filter(r -> odiSelbstzahlende.getId().equals(r.getOrtDerImpfung().getId()))
				.findAny()
				.orElse(null);
	...
	private Map<OrtDerImpfung, Long> getSelbstzahlendeRegistrierungsimpfungen(
		...
		BooleanExpression regJoinExporession, BooleanTemplate booleanTemplate) {
	...
	public List<AbrechnungZHDataRow> findOdiAbrechnungZH(
		...
		QImpfslot slot = new QImpfslot("slot");
		QImpfung impfung = new QImpfung("impfung");
		QRegistrierung registrierung = new QRegistrierung("registrierung");
		...
		BooleanExpression regJoinExporessionT1 = registrierung.impftermin1.eq(termin);
		List<AbrechnungZHDTO> result =
			runOdiAbrechnungZHRegistrierungsimpfungenQuery(
				dateVon, dateBis,
				termin, impfung, odi, slot,
				booleanTemplate, regJoinExporessionT1,
				"Impfungen 1");

		BooleanExpression regJoinExporessionT2 = registrierung.impftermin2.eq(termin);
		result.addAll(runOdiAbrechnungZHRegistrierungsimpfungenQuery(
			dateVon, dateBis,
			termin, impfung, odi, slot,
			booleanTemplate, regJoinExporessionT2,
			"Impfungen 2"));
	...
	private List<AbrechnungZHDTO> runOdiAbrechnungZHRegistrierungsimpfungenQuery(
		...
		List<AbrechnungZHDTO> result =
			getAbrechnungZHRootQuery(dateVon, dateBis, qOdi, qTermin, qSlot, qImpfung, booleanTemplate)
				.innerJoin(registrierung).on(regJoinExpression)
				.fetch();
	...
	private BooleanExpression getBooleanExpImpftermin1() {
		return registrierung.impftermin1.eq(QImpftermin.impftermin);
	}

	private BooleanExpression getBooleanExpImpftermin2() {
		return registrierung.impftermin2.eq(QImpftermin.impftermin);
	}
