...
public class DataMigrationService {
	...
	public String migrateData(RegistrierungMigrationJax request) {
		...
		Registrierung registrierung = getOrCreateRegistrierung(request);
		final RegistrierungStatus oldStatus = registrierung.getRegistrierungStatus();
		...
		sortedList.forEach(impfRequest -> {
			...
			if (Impffolge.ERSTE_IMPFUNG == impfRequest.getImpffolge()) {
				Impftermin oldTermin1 = registrierung.getImpftermin1();
				Impftermin termin1 = getOrCreateImpfterminForMigration(registrierung, impfRequest);
				...
				Impftermin oldTermin2 = registrierung.getImpftermin2();
				Impftermin termin2 = getOrCreateImpfterminForMigration(registrierung, impfRequest);
				...
				if (registrierung.getImpftermin1() != null) {
					...
					registrierung.setStatusToAbgeschlossen(infos, impfung2);
				}

				Optional.ofNullable(registrierung.getImpftermin1()).orElseThrow(() -> new AppFailureException("Bei der zweiten Impfung muss zwingend ein "
		...
		if (request.getVollstaendigerImpfschutz() != null && request.getVollstaendigerImpfschutz()) {
			if (registrierung.getImpftermin2() != null) {
				Impfung impfung2 = impfungRepo.getByImpftermin(registrierung.getImpftermin2()).get();
				final ImpfinformationDto infos =
					impfinformationenService.getImpfinformationenNoCheck(registrierung.getRegistrierungsnummer());
				registrierung.setStatusToAbgeschlossen(infos, impfung2); // wenn 2 impfungen vorhanden sind ignorieren wir das flag
			} else {
		...
		if (registrierung.getImpftermin1() != null && registrierung.getImpftermin2() != null) {
			ValidationUtil.validateFirstTerminBeforeSecond(registrierung.getImpftermin1(), registrierung.getImpftermin2());
		}
	...
	@NotNull
	private Registrierung getOrCreateRegistrierung(@NotNull RegistrierungMigrationJax request) {
		...
		return registrierungRepo.getByExternalId(request.getExternalId()).get();
	}
	...
	private Fragebogen deleteAndCreateFragebogen(@NotNull RegistrierungMigrationJax request, @NotNull Registrierung registrierung) {
		...
		Personenkontrolle personenkontrolle = null;
		if (fragebogenDB.isPresent()) {
			personenkontrolle = fragebogenDB.get().getPersonenkontrolle();
			fragebogenRepo.delete(fragebogenDB.get().toId());
		}
		...
		if (personenkontrolle != null) {
			fragebogen.setPersonenkontrolle(personenkontrolle);
		}
	...
	@NotNull
	private Impftermin getOrCreateImpfterminForMigration(@NotNull Registrierung registrierung, @NotNull ImpfungMigrationJax impfRequest) {
		final OrtDerImpfung odi = ortDerImpfungService.getByOdiIdentifier(mapDummyODITyp(impfRequest.getOrtDerImpfungTyp()));
		Impftermin oldTermin = Impffolge.ERSTE_IMPFUNG == impfRequest.getImpffolge() ? registrierung.getImpftermin1() : registrierung.getImpftermin2();
		if (oldTermin == null ||
			...
			Impftermin termin = terminbuchungService.createOnDemandImpftermin(impfRequest.getImpffolge(), odi,
				impfRequest.getImpfdatum().atTime(LocalTime.NOON));
			replaceTermin(registrierung, impfRequest, oldTermin, termin);
		}
		return Objects.requireNonNull(Impffolge.ERSTE_IMPFUNG == impfRequest.getImpffolge() ? registrierung.getImpftermin1() : registrierung.getImpftermin2());
	}
	...
	private void replaceTermin(@NotNull Registrierung registrierung, @NotNull ImpfungMigrationJax impfRequest, @Nullable Impftermin oldTermin,
	...
	public void patchPLZ(RegistrierungPLZPatchMigrationJax request) {
		...
		resetVMDLFlag(registrierung.getImpftermin1());
		resetVMDLFlag(registrierung.getImpftermin2());
	}
	...
	public String patchGLN(RegistrierungGLNPatchMigrationJax request) {
		...
		request.getImpfungen().forEach(impfungRequest -> {
			...
			final ImpfungOdiKorrekturJax korrekturJax = new ImpfungOdiKorrekturJax(impfungRequest.getImpffolge(), odi.getId(), null);
			// Impfung suchen
			Impfung impfung = null;
			if (impfungRequest.getImpffolge() == Impffolge.ERSTE_IMPFUNG) {
				if (registrierung.getImpftermin1() != null) {
					impfung = korrekturService.getImpfungByTermin(registrierung.getImpftermin1());
				}
			} else if (impfungRequest.getImpffolge() == Impffolge.ZWEITE_IMPFUNG) {
				if (registrierung.getImpftermin2() != null) {
					impfung = korrekturService.getImpfungByTermin(registrierung.getImpftermin2());
				}
			} else {
			...
			korrekturService.impfungOdiKorrigieren(korrekturJax, impfung.getTermin(), impfinformationen);
		});
	...
	public void claimRegistration(String regNum, Benutzer currentBenutzer) {
		var registration = registrierungRepo.getByRegistrierungnummer(regNum)
			.orElseThrow(NotFoundException::new);

		registration.setBenutzerId(currentBenutzer.getId());
		registration.setMail(currentBenutzer.getEmail());
		registration.setRegistrierungsEingang(RegistrierungsEingang.ONLINE_REGISTRATION);
		registration.setRegistrierungStatus(RegistrierungStatus.FREIGEGEBEN_BOOSTER);

		registrierungRepo.update(registration);
	}
}
