...
public class ImpfdossierJax {
	List<ImpfdossiereintragJax> impfdossiereintraege;
	List<ErkrankungJax> erkrankungen;

	@Nullable ImpfschutzJax impfschutzJax;
	...
	private void appendErkrankungFromGrundimmunisierung(@NonNull List<ErkrankungJax> list, @NotNull @NonNull Impfdossier impfdossier) {
		...
		if (registrierung.isGenesen() && registrierung.getPositivGetestetDatum() != null) {
			ErkrankungJax erkrankungFromGrundimmunisierungsprozess = new ErkrankungJax(registrierung.getPositivGetestetDatum(),
