...
public class GeimpftResource {
	...
	@RolesAllowed({ OI_DOKUMENTATION, KT_NACHDOKUMENTATION, KT_MEDIZINISCHE_NACHDOKUMENTATION, KT_IMPFDOKUMENTATION })
	public Response saveBemerkung(
		...
		ImpfinformationDto impfinformationen = this.impfinformationenService.getImpfinformationenNoCheck(registrierungsnummer);
		Registrierung registrierung = impfinformationen.getRegistrierung();
		authorizer.checkUpdateAuthorization(registrierung);

		if (impffolge == Impffolge.ZWEITE_IMPFUNG && registrierung.verzichtetOhneVollstaendigemImpfschutz()) {
			...
			if (!Objects.equals(bemerkung, registrierung.getZweiteImpfungVerzichtetGrund())) {
				registrierung.setZweiteImpfungVerzichtetGrund(bemerkung);
			}
		} else {
			if (registrierung.verzichtetOhneVollstaendigemImpfschutz()) {
