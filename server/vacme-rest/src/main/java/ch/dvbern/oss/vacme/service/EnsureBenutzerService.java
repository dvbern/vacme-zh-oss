...
public class EnsureBenutzerService {
	...
	public boolean ensureBenutzerActive() {
		...
		if (benutzer.isEmpty()) {
			...
			if (DateUtil.getMinutesBetween(creationDateTime, LocalDateTime.now())
				> getBenutzerDeactivateAfterInactiveTimeMinutes()) {
				keyCloakService.disableUser(uuid.toString());
				return false;
			}
		}
		return true;
	...
	public void ensureBenutzer() {
		...
		UpdateBenutzerDTO updateBenutzerDTO =
			new UpdateBenutzerDTO(preferred_username, familyName, givenName, email, mobileNum, glnNum, issuer);
