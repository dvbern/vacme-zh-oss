...
public class ImpfungDatumKorrekturJax {
	...
	public boolean needsNewZertifikat(
		...
		// vollstaendig war
		if (impfinformationDto.getRegistrierung().abgeschlossenMitVollstaendigemImpfschutz()) {
			...
			if (Impffolge.ERSTE_IMPFUNG == impffolge) {
				...
				if (hasSecondGrundimmunisierungsimpfung) {
					...
					boolean impfungAfterGenesung =
						!ImpfinformationenUtil.isRelevantImpfungBeforeCoronaTest(impfinformationDto.getRegistrierung(), terminTime);
	...
	public boolean needToRevoke(
		...
		final LocalDate pcrDatum = impfinformationDto.getRegistrierung().getPositivGetestetDatum();
		if (impfinformationDto.getRegistrierung().getVollstaendigerImpfschutzTyp() == VollstaendigerImpfschutzTyp.VOLLSTAENDIG_VACME_GENESEN
