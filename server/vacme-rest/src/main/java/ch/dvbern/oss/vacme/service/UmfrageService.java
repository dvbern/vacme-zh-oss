...
public class UmfrageService {
	...
	@Inject
	public UmfrageService(
		@NonNull StammdatenService stammdatenService,
		@NonNull UmfrageRepo umfrageRepo,
		@NonNull FragebogenService fragebogenService,
	...
	private boolean checkIfStatusStillOkay(@NonNull Umfrage umfrage) {
		...
		if (UmfrageGruppe.GRUPPE_1 == gruppe || UmfrageGruppe.GRUPPE_3 == gruppe || UmfrageGruppe.GRUPPE_5 == gruppe) {
			return !EnumUtil.isNoneOf(
				umfrage.getRegistrierung().getRegistrierungStatus(),
				REGISTRIERT, FREIGEGEBEN);
		}
		return !EnumUtil.isNoneOf(
			umfrage.getRegistrierung().getRegistrierungStatus(),
				...
				kandidaten.forEach(kand -> {
					...
					if (UmfrageGruppe.GRUPPE_2 == gruppe || UmfrageGruppe.GRUPPE_4 == gruppe || UmfrageGruppe.GRUPPE_6 == gruppe) {
						if (registrierung.getImpftermin1() != null) {
							impfung1 = impfungRepo.getByImpftermin(registrierung.getImpftermin1()).orElse(null);
						}
					}
					try {
