...
public class ImpfkontrolleResource {
	...
	public ImpfkontrolleJax registrieren(ImpfkontrolleJax impfkontrolleJax) {
		...
		ImpfkontrolleJax impfkontrolleJaxReloaded = find(created.getRegistrierungsnummer());
	...
	public Response verzichtenZweiteImpfung(
		...
		Fragebogen fragebogen = fragebogenService.findFragebogenByRegistrierungsnummer(zweiteImpfungVerzichtenJax.getRegistrierungsnummer());
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	@Path("wahrnehmenZweiteImpfung/{registrierungsnummer}")
	@RolesAllowed({ OI_KONTROLLE, OI_IMPFVERANTWORTUNG,
		KT_NACHDOKUMENTATION, KT_MEDIZINISCHE_NACHDOKUMENTATION, KT_IMPFDOKUMENTATION })
	...
	public ImpfkontrolleJax find(
		...
		ImpfinformationDto infos =
			impfinformationenService.getImpfinformationenAndEnsureStatusIsAktuell(fragebogen.getRegistrierung().getRegistrierungsnummer());
		Integer kontrolleNr = ImpfinformationenService.getCurrentKontrolleNr(infos);
		ImpfungkontrolleTermin kontrolleTerminOrNull = ImpfinformationenService.getCurrentKontrolleTerminOrNull(infos, fragebogen);
	...
	@RolesAllowed({ OI_KONTROLLE, KT_IMPFDOKUMENTATION })
	public ImpfkontrolleJax kontrolleOk(
		...
		Fragebogen fragebogen = fragebogenService.findFragebogenByRegistrierungsnummer(impfkontrolleJax.getRegistrierungsnummer());

		authorizer.checkReadAuthorization(fragebogen.getRegistrierung());

		Objects.requireNonNull(impfkontrolleJax.getExternGeimpft());
		impfkontrolleService.kontrolleOk(fragebogen, impffolge, null, impfkontrolleJax, impfkontrolleJax.getExternGeimpft());

		ImpfkontrolleJax impfkontrolleJaxReloaded = find(fragebogen.getRegistrierung().getRegistrierungsnummer());
		return impfkontrolleJaxReloaded;
	}
	...
	public RegistrierungsCodeJax saveKontrolleNoProceed(
		...
		final ImpfungkontrolleTermin impfungkontrolleTermin =
			impfkontrolleService.updateImpfkontrolleTermine(fragebogen, impfkontrolleJax, impffolge);
