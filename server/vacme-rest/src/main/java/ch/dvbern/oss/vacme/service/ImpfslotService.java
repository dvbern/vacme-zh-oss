...
public class ImpfslotService {
	...
	public void createEmptyImpfslots(@NonNull OrtDerImpfung ortDerImpfung, @NonNull LocalDateTime startDay, int monthsAmount) {
		...
		Set<LocalDateTime> alreadyExistsslots = impfslotRepo
			.find(ortDerImpfung, startDay.toLocalDate(),startDay.plusMonths(monthsAmount).toLocalDate() )
			.stream()
			.map(Impfslot::getZeitfenster)
			.map(DateTimeRange::getVon)
			.collect(Collectors.toSet());
		LOG.debug(String.format("Bestehende Impfslot gefunden (total):(%d)", alreadyExistsslots.size()));
		...
		getSlotStream(startDay, startDay.plus(Period.ofMonths(monthsAmount)), slotDuration, filter)
			...
			.forEach(d -> {
				LOG.trace(String.format("Erstelle Impfslot fuer Zeitraum (%1$td-%1$tm-%1$tY %1$tT - %2$td-%2$tm-%2$tY "
					+ "%2$tT)", d, d.plus(slotDuration)));
				Impfslot impfslot = Impfslot.of(ortDerImpfung, DateTimeRange.of(d, d.plus(slotDuration)));
				impfslotRepo.create(impfslot);
	...
	private void removeImpftermineForChangedKapazitaet(@NonNull Impfslot impfslot, Impffolge impffolge, int anzahl) {
		...
		for (int i = 0, impftermineSize = impftermine.size(); i < impftermineSize; i++) {
			...
			if (Impffolge.BOOSTER_IMPFUNG.equals(impftermin.getImpffolge())) {
				...
				empty = registrierungRepo.getRegistrierungForGrundimpftermin(impftermin).isEmpty();
			}
	...
	public List<Impfslot> find(@NonNull OrtDerImpfung ortDerImpfung) {
		return impfslotRepo.find(ortDerImpfung);
	}
	...
	public List<Impfslot> find(
		...
		return impfslotRepo.find(ortDerImpfung, dateVon, dateBis);
	}

	@Nullable
	public Impfslot findImpfslotForTime(@NonNull OrtDerImpfung ortDerImpfung, LocalDateTime desiredTime) {
		return impfslotRepo.find(ortDerImpfung, desiredTime);
	}

	public boolean hasAtLeastFreieImpftermine(Integer minTermin) {
		return impfterminRepo.hasAtLeastFreieImpfslots(minTermin);
	}
