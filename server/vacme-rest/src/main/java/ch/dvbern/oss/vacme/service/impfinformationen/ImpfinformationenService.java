...
public class ImpfinformationenService {
	...
	@NonNull
	public ImpfinformationDto getImpfinformationenAndEnsureStatusIsAktuell(@NonNull String registrierungsnummer) {
		ImpfinformationDto infos = getImpfinformationenNoCheck(registrierungsnummer);
		registrierungService.ensureStatusIsAktuell(infos.getRegistrierung());
		return infos;
	}

	@NonNull
	public ImpfinformationDto getImpfinformationenNoCheck(@NonNull String registrierungsnummer) {
		ImpfinformationDto infos = impfungRepo.getImpfinformationenOptional(registrierungsnummer).orElseThrow(
	...
	@Nullable
	public static Impfung readImpfungForImpffolgeNr(@NonNull ImpfinformationDto infos, @NonNull Impffolge impffolge, @NonNull Integer impffolgeNr) {
	...
	@Nullable
	public static Impfung getImpfungForEintrag(@NonNull ImpfinformationDto infos, @NonNull Impfdossiereintrag eintrag) {
	...
	@Nullable
	public static Impfdossiereintrag getDossiereintragForNr(@NonNull ImpfinformationDto infos, @NonNull Integer impffolgeNr) {
	...
	public static int getNumberOfImpfung(@NonNull ImpfinformationDto infos) {
		return ImpfinformationenService.getNumberOfImpfung(infos.getImpfung1(), infos.getImpfung2(), infos.getBoosterImpfungen(),
	...
	@Nullable
	public static ImpfungkontrolleTermin getCurrentKontrolleTerminOrNull(@NonNull ImpfinformationDto infos, @NonNull Fragebogen fragebogen) {
		switch (infos.getRegistrierung().getRegistrierungStatus()) {
		// Kontrolle 1
		case REGISTRIERT:
		case FREIGEGEBEN:
		case GEBUCHT:
	...
	@NonNull
	public static Optional<Impfdossiereintrag> getImpfdossierEintragWithID(@NonNull ImpfinformationDto infos, @NonNull UUID eintragId) {
		if (infos.getImpfdossier() == null) {
			return Optional.empty();
		}
	...
	@Nullable
	public static Impftermin getImpftermin(@NonNull ImpfinformationDto infos, @NonNull Impffolge impffolge, @Nullable Integer impffolgeNr) {
		// Der Termin kann leer sein (wenn die Person einfach am ODI erscheint ohne Termin)
		switch (impffolge) {
		case ERSTE_IMPFUNG:
			return infos.getRegistrierung().getImpftermin1();
		case ZWEITE_IMPFUNG:
			return infos.getRegistrierung().getImpftermin2();
		case BOOSTER_IMPFUNG:
			if (infos.getImpfdossier() == null || impffolgeNr == null) {
				return null;
			}
	...
	public static int getNumberOfImpfung(
		...
		if (boosterImpfungen != null) {
			result += boosterImpfungen.size();
		}

		if (externesZertifikat != null) { // Die Impfungen des Externen Zertifikats zaehlen auch, wenn es nicht vollstaendig ist (Realitaet abbilden)
	...
	public static int getImpffolgeNrOfImpfung1Or2(@Nullable ExternesZertifikat externesZertifikat, @NonNull Impffolge impffolge) {
		int result = 0;
	...
	@Nullable
	public static Impfung getNewestVacmeImpfung(@Nullable Impfung impfung1, @Nullable Impfung impfung2, @Nullable List<Impfung> boosterimpfungen) {
	...
	@Nullable
	public static ImpfungkontrolleTermin getNewestImpfkontrolleTermin(@NonNull ImpfinformationDto infos, @NonNull Fragebogen fragebogen) {
		...
		if (infos.getImpfdossier() != null) {
			Optional<Impfdossiereintrag> newestImpfdossierEintrag = infos.getImpfdossier().getImpfdossierEintraege().stream().max(Comparator.naturalOrder());
			if (newestImpfdossierEintrag.isPresent() && newestImpfdossierEintrag.get().getImpfungkontrolleTermin() != null) {
	...
	private Impfdossier getOrCreateImpfdossier(@NonNull ImpfinformationDto impfinformationen) {
		Registrierung registrierung = impfinformationen.getRegistrierung();
		return impfdossierRepo.findImpfdossierForReg(registrierung)
			.orElseGet(() -> {
				Impfdossier newDossier = new Impfdossier();
				newDossier.setRegistrierung(registrierung);
				impfdossierRepo.create(newDossier);
				return newDossier;
			});
	}
	...
	public Impfdossiereintrag getExistingLatestImpfdossierEintrag(
		...
		if (!impffolgeNr.equals(getNumberOfImpfung(impfinformationen) + 1)) {
			throw new AppFailureException("Die ubergebene ImpffolgeNr " + impffolgeNr + " entspicht nicht der detektierten Anzahl Impfungen");
		}
		Impfdossier impfdossier = getOrCreateImpfdossier(impfinformationen);
	...
	public boolean hasVacmeImpfungen(@NonNull @NotNull Registrierung registrierung) {
		return hasVacmeImpfungen(getImpfinformationenNoCheck(registrierung.getRegistrierungsnummer()));
	}

	public boolean hasVacmeImpfungenOrVollstaendigerImpfschutz(@NonNull @NotNull Registrierung registrierung) {
		return hasVacmeImpfungen(registrierung) || Boolean.TRUE.equals(registrierung.getVollstaendigerImpfschutz());
	}
		...
		case BOOSTER_IMPFUNG:
			if (dossiereintragID != null) {
				...
				if (existingDossiereintragOptional.isPresent()) {
					return Optional.ofNullable(ImpfinformationenService.getImpfungForEintrag(impfinformationen, existingDossiereintragOptional.get()));
				}
			}
			return Optional.empty();
	...
	public static boolean willBeGrundimmunisiertAfterErstimpfung(@NonNull @NotNull Impfung erstImpfung, @NonNull @NotNull ImpfinformationDto infos) {
		return ImpfinformationenUtil.willBeGrundimmunisiertAfterErstimpfungImpfstoff(erstImpfung.getImpfstoff(), infos.getExternesZertifikat());
	}
}
