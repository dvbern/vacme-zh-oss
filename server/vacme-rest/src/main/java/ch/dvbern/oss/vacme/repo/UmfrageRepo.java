...
public class UmfrageRepo {
	...
	public List<UmfrageDTO> getKandidatenUmfrage3(int limit) {
		final Set<RegistrierungStatus> statusList = Set.of(
			REGISTRIERT,
			FREIGEGEBEN);
	...
	@NonNull
	public List<UmfrageDTO> getKandidatenErwachsene(@NonNull Set<RegistrierungStatus> statusList, boolean vorStichtag, int limit) {
		if (vorStichtag) {
			// vorStichtag heisst eigentlich: Registrierungstimestamp egal
			return db
				.select(Projections.constructor(UmfrageDTO.class,
					registrierung,
				...
				.where(registrierung.id.in(getUniqueOnlineErwachsene())
					.and(registrierung.registrierungStatus.in(statusList)))
