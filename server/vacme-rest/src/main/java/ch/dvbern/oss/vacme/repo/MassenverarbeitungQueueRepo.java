...
public class MassenverarbeitungQueueRepo {
	...
	public void addImpfungenToExternalizeQueue(@NonNull List<ID<Impfung>> impfungIdsToExternalize) {
		...
		List<MassenverarbeitungQueue> itemsToInsert = transformImpfungIdstoQueueItemsForExternalization(impfungIdsToExternalize);
		LOG.info("VACME-MASSENVERARBEITUNG: Starting to insert {} MassenverarbeitungQueue Items for Externalization ...",
	...
	private int createQueueItemsJPA(@NonNull List<ID<Impfung>> impfungIdsToExternalize) {
		List<MassenverarbeitungQueue> itemsToStore = transformImpfungIdstoQueueItemsForExternalization(impfungIdsToExternalize);
	...
	private List<MassenverarbeitungQueue> transformImpfungIdstoQueueItemsForExternalization(@NonNull List<ID<Impfung>> impfungIdsToExternalize) {
		List<MassenverarbeitungQueue> itemsToStore =
			impfungIdsToExternalize.stream().map(MassenverarbeitungQueue::forExternalization).collect(Collectors.toList());
		return itemsToStore;
	}
	...
	private int createQueueItemsJDBC(@NonNull List<MassenverarbeitungQueue> itemsToInsert) {
		...
		String query = "INSERT INTO %s (id, timestampErstellt, timestampMutiert, userErstellt, userMutiert, version, "
			+ "errorCount, lastError, impfungId, odiId, registrierungNummer, status, typ) "
		...
		try (Connection connection = defaultDataSource.getConnection();
			...
			for (MassenverarbeitungQueue item : itemsToInsert) {
				...
				if ((counter + 1) % batchSize == 0 || (counter + 1) == itemsToInsert.size()) {
					...
					LOG.info("VACME-MASSENVERARBEITUNG: Triggered batch-insert after processing {}/{}", counter, itemsToInsert.size());
				}
				counter++;
			}
	...
	public void addImpfungenToMoveToOdiQueue(@NonNull List<Pair<ID<Impfung>, ID<OrtDerImpfung>>> impfungenToMoveToOdiList) {
		StopWatch started = StopWatch.createStarted();
		LOG.info("VACME-MASSENVERARBEITUNG: Starting to insert {} MassenverarbeitungQueue Items for OdiMove ...",
	...
	public void addImpfungenToChangeVerantwortlicherQueue(
		@NonNull List<Pair<ID<Impfung>, ID<Benutzer>>> impfungenToChangeVerantwortlicherList
	) {
		StopWatch started = StopWatch.createStarted();
		LOG.info("VACME-MASSENVERARBEITUNG: Starting to insert {} MassenverarbeitungQueue Items for VerantwortlicherChange ...",
			impfungenToChangeVerantwortlicherList.size());
		int i = createQueueItemsJDBC(transformImpfungIdstoQueueItemsForImpfungChangeVerantwortlicher(
			impfungenToChangeVerantwortlicherList));
		LOG.info("VACME-MASSENVERARBEITUNG: Added {} queue items in  {} ms", i,
			started.getTime(TimeUnit.MILLISECONDS));
	}
	...
	public void addRegistrierungenToDeleteQueue(List<String> registrierungsnummern) {
		StopWatch started = StopWatch.createStarted();
		LOG.info("VACME-MASSENVERARBEITUNG: Starting to insert {} MassenverarbeitungQueue Items for RegistrationDelete ...",
			registrierungsnummern.size());
		int i = createQueueItemsJDBC(transformRegistrierungsnummernToQueueItemsForRegistrierungDelete(registrierungsnummern));
		LOG.info("VACME-MASSENVERARBEITUNG: Added {} queue items in  {} ms", i,
			started.getTime(TimeUnit.MILLISECONDS));
	}

	public void addRegistrierungenToCreateImpfdossierQueue(List<String> registrierungsnummern) {
		StopWatch started = StopWatch.createStarted();
		LOG.info("VACME-MASSENVERARBEITUNG: Starting to insert {} MassenverarbeitungQueue Items for Impfdossier creation ...",
			registrierungsnummern.size());
		int i = createQueueItemsJDBC(transformRegistrierungsnummernToQueueItemsForImpfdossierCreation(registrierungsnummern));
		LOG.info("VACME-MASSENVERARBEITUNG: Added {} queue items in  {} ms", i,
			started.getTime(TimeUnit.MILLISECONDS));
	}
	...
	public void addImpfungenToLoeschenQueue(@NonNull List<ID<Impfung>> impfungIdsToLoeschen) {
		StopWatch started = StopWatch.createStarted();
		List<MassenverarbeitungQueue> itemsToDelete = transformImpfungIdsToQueueItemsForImpfungenLoeschen(impfungIdsToLoeschen);
		LOG.info("VACME-MASSENVERARBEITUNG: Starting to insert {} MassenverarbeitungQueue Items for ImpfungenLoeschen ...",
	...
	private List<MassenverarbeitungQueue> transformImpfungIdsToQueueItemsForImpfungenLoeschen(@NonNull List<ID<Impfung>> impfungIdsToLoeschen) {
		List<MassenverarbeitungQueue> itemsToStore =
			impfungIdsToLoeschen.stream().map(MassenverarbeitungQueue::forImpfungenLoeschen).collect(Collectors.toList());
		return itemsToStore;
	}

	@NonNull
	private List<MassenverarbeitungQueue> transformRegistrierungsnummernToQueueItemsForImpfdossierCreation(@NonNull List<String> registrierungsnummern) {
		return registrierungsnummern.stream()
			.map(MassenverarbeitungQueue::forRegistrierungEnsureImpfdossier)
			.collect(Collectors.toList());
	}

	@NonNull
	private List<MassenverarbeitungQueue> transformImpfungIdstoQueueItemsForOdiMove(@NonNull List<Pair<ID<Impfung>, ID<OrtDerImpfung>>> impfungenToMoveToOdiList) {
		return impfungenToMoveToOdiList.stream()
			.map(impfungToNewOdiPair -> MassenverarbeitungQueue.forImpfungOdiMove(impfungToNewOdiPair.getLeft(),
				impfungToNewOdiPair.getRight()))
			.collect(Collectors.toList());
	}

	@NonNull
	private List<MassenverarbeitungQueue> transformImpfungIdstoQueueItemsForImpfungChangeVerantwortlicher(
		@NonNull List<Pair<ID<Impfung>, ID<Benutzer>>> impfungenToChangeVerantwortlicherList
	) {
		return impfungenToChangeVerantwortlicherList.stream()
			.map(impfungToNewOdiPair -> MassenverarbeitungQueue.forImpfungChangeVerantwortlicher(impfungToNewOdiPair.getLeft(),
				impfungToNewOdiPair.getRight()))
			.collect(Collectors.toList());
	}
	...
	public List<MassenverarbeitungQueue> findMassenverarbeitungQueueItemsToProcess(
		...
		return db.select(QMassenverarbeitungQueue.massenverarbeitungQueue)
			...
			.where(QMassenverarbeitungQueue.massenverarbeitungQueue.typ.eq(typ)
				.and(QMassenverarbeitungQueue.massenverarbeitungQueue.status.in(MassenverarbeitungQueueStatus.NEW, MassenverarbeitungQueueStatus.FAILED_RETRY)))
