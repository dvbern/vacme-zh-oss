...
public class OnboardingResource {
	...
	public Response startOnboardingBeforeKeycloakRegistration(StartOnboardingJax startOnboardingJax) {
		...
		Onboarding validatedOnboarding = onboardingService.startOnboardingValidateAndIncreaseNumOfTries(startOnboardingJax.getOnboardingcode(),
		...
		int cookieTimeoutSeconds = onboardingTokenTTLMinutes * 60;
		NewCookie onboardingTokenCookie = new NewCookie(ONBOARDING_TOKEN_COOKIE_NAME, token,
			"/", null, null, cookieTimeoutSeconds, cookieSecure, true);
	...
	public Response finishOnboardingAfterKeycloakRegistration(
		...
		if (userPrincipal.userHasRoleOtherThanImpfwilliger(currentBenutzer.getRoles())) {
			LOG.error("VACME-ONBOARDING-COMPLETION Benutzer hat nicht nur Impfwilliger Rolle. Dies ist nicht erwartet");
	...
	public Response triggerOnboardingLetter(
		...
		// Validieren, dass die Reg in einem richtigen Status ist (mind. einmal geimpft)
		boolean wrongState = ValidationUtil.invalidOnboardingState(registrierung.getRegistrierungStatus());
		if (wrongState) {
			LOG.error("VACME-ONBOARDING: Registration {} was in wrong state for triggered onboarding letter {}",
				registrierung.getRegistrierungsnummer(), registrierung.getRegistrierungStatus());
			throw AppValidationMessage.ONBOARDING_CREATION_FAILED.create("invalid registration status {}", registrierung.getRegistrierungStatus());
		}
