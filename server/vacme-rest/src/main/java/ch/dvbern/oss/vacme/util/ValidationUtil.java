...
import static ch.dvbern.oss.vacme.shared.util.Constants.MIN_DATE_FOR_IMPFUNGEN;
...
public final class ValidationUtil {
	...
	public static void validateStatus(
		...
		@NonNull RegistrierungStatus expectedStatus
	) {
		if (expectedStatus != registrierung.getRegistrierungStatus()) {
			throw AppValidationMessage.REGISTRIERUNG_WRONG_STATUS.create(registrierung.getRegistrierungsnummer(), expectedStatus);
		}
	}

	public static void validateNotArchiviert(@NonNull Registrierung registrierung) {
		if (registrierung.getTimestampArchiviert() != null) {
			throw AppValidationMessage.REGISTRIERUNG_WRONG_STATUS.create(registrierung.getRegistrierungsnummer(), "NOT ARCHIVIERT");
		}
	}

	public static void validateCanGetZertifikat(@NonNull Registrierung registrierung) {
		if (registrierung.isAnonymisiert() ||
			!registrierung.abgeschlossenMitVollstaendigemImpfschutz() || // kein vollstaendiger impfschutz
			registrierung.isAbgleichElektronischerImpfausweis() != Boolean.TRUE ||
			registrierung.getRegistrierungsEingang() == RegistrierungsEingang.UNBEKANNT) {
	...
	public static void validateStatusOneOf(
		...
		if (Arrays.stream(expectedStatus).noneMatch(registrierungStatus -> registrierungStatus == registrierung.getRegistrierungStatus())) {
			throw AppValidationMessage.REGISTRIERUNG_WRONG_STATUS.create(registrierung.getRegistrierungsnummer(), expectedStates);
		}
	}
	...
	public static void validateAnzahlImpfungen(@NonNull Impffolge impffolge, @NonNull ImpfinformationDto impfinformationen) {
		switch (impffolge) {
		case ERSTE_IMPFUNG:
	 ...
	 */
	public static void validateSecondImpfungOnSameDay(@NonNull Impfung firstImpfung, @NonNull LocalDateTime timestamp) {
	 ...
	 * too early for the next vaccinations
	 */
	public static void validateSecondImpfungBeforeFirst(@NonNull ImpfInfo firstImpfung, @NonNull LocalDateTime timestamp) {
		validateNewImpfungNotBeforePrevious(firstImpfung, timestamp, AppValidationMessage.IMPFTERMIN_IMPFUNG_BEFORE_FIRST);
	}

	public static void validateNewImpfungNotBeforePrevious(@NonNull ImpfInfo prevImpfung, @NonNull LocalDateTime timestamp,
		@NonNull AppValidationMessage appMsg1Param) {
		LocalDateTime timestampImpfung = prevImpfung.getTimestampImpfung();
		if (timestamp.isBefore(timestampImpfung)) {
	...
	/**
	 * Unsere erste ImpffolgeNr muss eine Nummer hoeher sein als die Anzahl Impfungen die in einem allfaellig vorhandenen
	 * ExternenZertifikat erfasst wurden
	 */
	private static void validateImpfolgeNrOfFirstImpfungIfExternesZertifikatPresent(Integer firstVacmeImpffolgeNr, ImpfinformationDto impfinformationDto) {
	...
	private static int findIndexOfImpfungInImpfInfoList(
		...
		throw AppValidationMessage.ILLEGAL_STATE.create("Zu korrigierende Impfung muss in Impfinfoliste vorhanden sein");
	}
	...
	public static void validateNewImpfungNotBeforePreviousTimestamp(
		...
		LocalDateTime timestampImpfung = prevImpfung.getTimestampImpfung();
		if (!timestamp.isAfter(timestampImpfung)) {
	...
	public static void validateBoosterImpfungEinTagNachBisherige(
		...
		List<Impfung> previousImpfungen = Lists.newArrayList(ImpfinformationenUtil.getImpfungenOrderedByImpffolgeNr(impfinformationen));
	...
	public static void validateDaysBetweenImpfungen(
		...
		if (between < minimumDaysBetweenImpfungen) {
			// wir sollten nie in die Situation kommen diesen Logoutput printen zu muessen, soll auf gui abgefangen werden
	...
	public static void validateFirstTerminBeforeSecond(@Nullable Impftermin impftermin1, @Nullable Impftermin impftermin2) {
	...
	public static void validateImpfungMinDate(@NonNull Impftermin impftermin) {
		LocalDateTime terminDate = impftermin.getImpfslot().getZeitfenster().getVon();
		if (terminDate.isBefore(MIN_DATE_FOR_IMPFUNGEN)) {
			String msg = String.format("Termin can not be before start of vaccination campaign %s", MIN_DATE_FOR_IMPFUNGEN);
	...
	public static boolean invalidOnboardingState(RegistrierungStatus status) {

		// Fuer ein Onboarding muss man mindestens einmal geimpft worden sein, deshalb sind alle vorherigen States ungueltig
		boolean wrongState = Stream.of(REGISTRIERT, FREIGEGEBEN, ODI_GEWAEHLT, GEBUCHT, IMPFUNG_1_KONTROLLIERT)
			.anyMatch(registrierungStatus -> status == registrierungStatus);
	...
	public static void validateImpfstoffZulassung(Impfstoff impfstoff, boolean extern) {
		...
		case EXTERN_ZUGELASSEN:
			if (!extern) { // nur extern zugelassen, ungueltig fuer intern
				LOG.error("VACME-ERROR: Es wurde versucht einen Imfpstoff ({}) zu verwenden der nur fuer externe zugelassen ist", impfstoff.getDisplayName());
	...
	public static void validateGrundimmunisierung(
		...
		boolean isExternGrundimmunisiert
	) {
		...
		if (!impfung.isGrundimmunisierung()) {
			if (impffolge == Impffolge.ERSTE_IMPFUNG || impffolge == Impffolge.ZWEITE_IMPFUNG) {
				// Impfung 1 und 2 muessen zur Grundimmunisierung gehoeren (wenn nur 1 noetig, ist eine spaetere 2. bereits in der Booster-Liste)
				throw AppValidationMessage.IMPFUNG_GRUNDIMMUNISIERUNG_INVALID.create("Die Impfung für die erste/zweite Impffolge muss zur Grundimmunisierung "
					+ "gehören");
			}
		...
		if (previousImpfung == null) {
			if (impffolge != Impffolge.ERSTE_IMPFUNG && !isExternGrundimmunisiert) {
				throw AppValidationMessage.IMPFUNG_GRUNDIMMUNISIERUNG_INVALID.create("Die vorherige Impfung wurde nicht gefunden");
			}
			return; // es ist die erste Impfung, das ist korrekt
		}
		if (!previousImpfung.isGrundimmunisierung()) {
			// die vorherige Impfung muss auch zur Grundimm. gehoeren
			throw AppValidationMessage.IMPFUNG_GRUNDIMMUNISIERUNG_INVALID.create("Die vorherige Impfung gehoert nicht zur Grundimmunisierung");
		}
	}
	...
	public static void validateCurrentKontrolleHasNoImpfungYet(
		...
		// Szenario: (Booster) Kontrolle in 2 Browser oeffnen, dann in Browser 1 Kontrolle und Impfung machen, dann in Browser 2 Kontrolle ok -> soll Fehler
		// geben
		if (ImpfinformationenService.getImpfungForKontrolle(impfinformationen, impffolge, dossiereintragID).isPresent()) {
