...
public class TestdataCreationResource {
	...
	@Inject
	public TestdataCreationResource(
		@NonNull TestdataCreationService testdataCreationService){
		this.testdataCreationService = testdataCreationService;
	}
	...
	public Response createRegistrierungenMitTermin(
		...
		try {
			isTestdataCreationAllowed();
			LOG.info("Creating Testdata...");
			final List<String> result = testdataCreationService.createRegistrierungenMitTermin(anzahl, odiname, datumTermin1, datumTermin2);
			LOG.info("...Testdata beendet");
			return Response.ok(result).build();
		} catch (Exception e) {
			LOG.error("Testdaten konnten nicht erstellt werden", e);
	...
	public Response createRegistrierungenForOnboarding(
		...
		try {
			isTestdataCreationAllowed();
			LOG.info("Creating Testdata...");
			final List<String> result = testdataCreationService.createRegistrierungenForOnboarding(anzahl, odiname, datumTermin1, datumTermin2);
			LOG.info("...Testdata beendet");
			return Response.ok(result).build();
		} catch (Exception e) {
			LOG.error("Testdaten konnten nicht erstellt werden", e);
