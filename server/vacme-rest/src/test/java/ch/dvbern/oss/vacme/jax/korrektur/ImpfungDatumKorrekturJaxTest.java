...
// neu: vorher PCR Test nache der Impfung, nachher PCR Test vor der Impfung -> createZert
	...
	void testDateChangeLeadsToNewCert(
		...
		ImpfungDatumKorrekturJax korrekturJax = new ImpfungDatumKorrekturJax(correctedFolge, newDate, null);


		Registrierung reg = new Registrierung();
		reg.setVollstaendigerImpfschutzFlagAndTyp(TestdataCreationUtil.guessVollstaendigerImpfschutzTyp(vollstaendigGeimpft,
			status));
		reg.setRegistrierungStatus(status);
		reg.setPositivGetestetDatum(genesenTestDatum);
		...
		if (RegistrierungStatus.ABGESCHLOSSEN == status) {
		...
		ImpfinformationDto impfinformationDto = new ImpfinformationDto(reg, impfung1, impfung2, null, null);
