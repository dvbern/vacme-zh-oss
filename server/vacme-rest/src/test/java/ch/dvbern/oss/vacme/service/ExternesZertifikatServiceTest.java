...
public class ExternesZertifikatServiceTest {
	ExternesZertifikatService externesZertifikatService;
	ExternesZertifikatRepo externesZertifikatRepo;
	ImpfstoffService impfstoffService;

	@Nullable
	ExternesZertifikat persistedZertifikat;
	...
	public void testIsGrundimmunisiert() {
		...
		externesZertifikat.setImpfstoff(TestImpfstoff.createImpfstoffForTest(TestImpfstoff.TEST_MODERNA));
		externesZertifikat.setAnzahlImpfungen(2);
		externesZertifikat.setGenesen(false);
		Assertions.assertTrue(externesZertifikat.isGrundimmunisiert());
	}
	...
	public void testIsGrundimmunisiert(boolean expectedGrundimmunisiert, TestImpfstoff testImpfstoff, Integer anzahlImpfungen, boolean genesen) {
		...
		Assertions.assertEquals(externesZertifikat.isGrundimmunisiert(), expectedGrundimmunisiert);
	}
	...
	public void testCreateUpdateRemove(
		...
		if (geimpftAlt) {
			...
			zertifikatAlt.setRegistrierung(registrierung);
		...
		Assertions.assertFalse(ExternesZertifikat.isGrundimmunisiert(impfstoffNeu, anzahlImpfungenNeu, genesenNeu, true));

		// createUpdateRemove
		ExternesZertifikat updatedZertifikat = externesZertifikatService.createUpdateOrRemoveExternGeimpft(registrierung, externGeimpftJaxNeu, false);
		...
		if (!geimpftAlt && geimpftNeu) {
			...
			Assertions.assertEquals(updatedZertifikat.getRegistrierung(), registrierung);
			...
			Assertions.assertEquals(updatedZertifikat.isGrundimmunisiert(), grundimmunisiertNeu);
		}
		// ja - nein
		if (geimpftAlt && !geimpftNeu) {
			Assertions.assertNull(updatedZertifikat);
	...
	public void testSaveImpfling(
		...
		// createUpdateRemove
		externesZertifikatService.saveExternGeimpftImpfling(registrierung, externGeimpftJaxNeu);
		...
		if (!geimpftAlt && geimpftNeu) {
			...
			Assertions.assertEquals(persistedZertifikat.getRegistrierung(), registrierung);
			Assertions.assertNotNull(impfstoffNeu);
			...
			Assertions.assertEquals(persistedZertifikat.isGrundimmunisiert(), grundimmunisiertNeu);
		}
		// ja - nein
		if (geimpftAlt && !geimpftNeu) {
			Assertions.assertNull(persistedZertifikat);
		...
		if (geimpftNeu && grundimmunisiertNeu) {
			Assertions.assertSame(Boolean.TRUE, registrierung.getVollstaendigerImpfschutz());
		} else {
			Assertions.assertNotSame(Boolean.TRUE, registrierung.getVollstaendigerImpfschutz());
		}
	...
	public void testImpfempfehlungGrundimmunisierung(
		...
		Assertions.assertEquals(
			...
			ExternesZertifikat.calculateAnzahlMissingImpfungen(impfstoff, anzahlImpfungenGemacht, genesen, null),
			testcase);
		Assertions.assertEquals(
			MissingForGrundimmunisiert.BRAUCHT_VOLLE_GRUNDIMMUNISIERUNG,
			ExternesZertifikat.calculateAnzahlMissingImpfungen(impfstoff, anzahlImpfungenGemacht, genesen, true),
			testcase);

		Assertions.assertEquals(
			expectedGrundimmunisiert,
			ExternesZertifikat.isGrundimmunisiert(impfstoff, anzahlImpfungenGemacht, genesen, null),
			testcase);
		Assertions.assertFalse(ExternesZertifikat.isGrundimmunisiert(impfstoff, anzahlImpfungenGemacht, genesen, true),
			testcase);
	}
}
