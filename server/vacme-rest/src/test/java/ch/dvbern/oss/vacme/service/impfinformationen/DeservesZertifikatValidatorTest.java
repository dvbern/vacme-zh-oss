...
class DeservesZertifikatValidatorTest {
	...
	void deservesZertifikat_vacme_covid_ohneTestdatum() {
		...
		infosNachCorona.getRegistrierung().setPositivGetestetDatum(null);
	...
	void assertDeservesZertifikat(@NonNull ImpfinformationDto infos, @Nullable Impfung impfung, boolean expected) {
		...
		Assertions.assertEquals(expected, DeservesZertifikatValidator.deservesZertifikat(infos.getRegistrierung(), impfung, infos.getExternesZertifikat()));
	}
