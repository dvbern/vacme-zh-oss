...
class CovidCertUtilTest {
	...
	public void testZahlVorUndNachSchraegstrich(ArgumentsAccessor accessor) {
		...
		Assertions.assertEquals(
			...
			Boolean.TRUE.equals(infos.getRegistrierung().getVollstaendigerImpfschutz()),
			"grundimmunisiert " + args.quelle);
		Assertions.assertEquals(
			args.vollstaendigerImpfschutzTyp,
			infos.getRegistrierung().getVollstaendigerImpfschutzTyp(),
			"vollstaendigerImpfschutzTyp " + args.quelle);

		final boolean deservesZertifikat = DeservesZertifikatValidator.deservesZertifikat(infos.getRegistrierung(),
			aktuelleImpfung,
	...
	private ImpfinformationDto setup(
		...
		registrierung.setGenesen(genesen);
		...
		if (impfstoffBooster1 != null) {
			impfdossier = createDossier(registrierung);
		...
		ImpfinformationDto infos = new ImpfinformationDto(new ImpfinformationDto(registrierung, impfung1, impfung2, impfdossier, externesZertifikat),
			boosterImpfungen);
		...
		if (vollstaendigerImpfschutzTyp1 != null) {
			registrierung.setVollstaendigerImpfschutzFlagAndTyp(vollstaendigerImpfschutzTyp1);
	...
	private VollstaendigerImpfschutzTyp calculateImpfschutzTyp(ImpfinformationDto infos) {
		...
		if (infos.getExternesZertifikat() != null && infos.getExternesZertifikat().isGrundimmunisiert()) {
		...
		if (infos.getImpfung1() != null) {
			ImpfinformationDto infos0 = new ImpfinformationDto(infos.getRegistrierung(), infos.getImpfung1(), null,
		...
		if (infos.getImpfung2() != null) {
			...
			if (infos.getRegistrierung().isGenesen()) {
	...
	@NonNull
	private Impfdossier createDossier(@NonNull Registrierung registrierung) {
		Impfdossier impfdossier;
		impfdossier = new Impfdossier();
