...
public class StatsServiceTest {
	...
	private final Impfstoff moderna = createImpfstoff("moderna", ZulassungsStatus.EMPFOHLEN, 2);
	private final Impfstoff pfizer = createImpfstoff("pfizer", ZulassungsStatus.EMPFOHLEN, 2);
	private final Impfstoff janssen = createImpfstoff("janssen", ZulassungsStatus.ZUGELASSEN, 1);
	...
	public void before() {
		...
		Mockito.when(statsRepoMock.getAllZugelassenenImpfstoffe()).thenReturn(List.of(moderna, pfizer, janssen));

		Mockito.doAnswer(invocation -> pendent1U).when(statsRepoMock).getPendentImpfung1(odi1, date);
		Mockito.doAnswer(invocation -> pendent2U).when(statsRepoMock).getPendentImpfung2ImpfstoffUnbekannt(odi1, date);
		Mockito.doAnswer(invocation -> pendentNU).when(statsRepoMock).getPendentImpfungNImpfstoffUnbekannt(odi1, date);

		Mockito.doAnswer(invocation -> pendent2.get(moderna)).when(statsRepoMock).getPendentImpfung2ImpfstoffEmpfohlen(moderna, odi1, date);
		Mockito.doAnswer(invocation -> pendent2.get(pfizer)).when(statsRepoMock).getPendentImpfung2ImpfstoffEmpfohlen(pfizer, odi1, date);
		Mockito.doAnswer(invocation -> pendent2.get(janssen)).when(statsRepoMock).getPendentImpfung2ImpfstoffEmpfohlen(janssen, odi1, date);
		Mockito.doAnswer(invocation -> pendentN.get(moderna)).when(statsRepoMock).getPendentImpfungNImpfstoffEmpfohlen(moderna, odi1, date);
		Mockito.doAnswer(invocation -> pendentN.get(pfizer)).when(statsRepoMock).getPendentImpfungNImpfstoffEmpfohlen(pfizer, odi1, date);
		Mockito.doAnswer(invocation -> pendentN.get(janssen)).when(statsRepoMock).getPendentImpfungNImpfstoffEmpfohlen(janssen, odi1, date);
