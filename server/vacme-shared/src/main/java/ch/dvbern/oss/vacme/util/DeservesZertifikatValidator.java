...
public final class DeservesZertifikatValidator {
	...
	public static boolean deservesZertifikatForAnyImpfung(@NonNull ImpfinformationDto infos) {
		...
		for (Impfung impfung : allImpfungen) {
			if (deservesZertifikat(infos.getRegistrierung(), impfung, infos.getExternesZertifikat())) {
				return true;
			}
		}
		return false;
	...
	public static void deservesZertifikatOrThrowException(@NonNull ImpfinformationDto infos, @Nullable Impfung impfung) {
		if (!deservesAndWantsZertifikat(infos.getRegistrierung(), impfung, infos.getExternesZertifikat())) {
	...
	public static boolean deservesZertifikat(
		...
		final boolean valid = CheckRegistrierungVollstaendigerImpfschutzValidator.isValid(registrierung, impfung);
		if (!valid) {
		...
		if (impfung.getTermin().getImpffolge() != Impffolge.BOOSTER_IMPFUNG
			&& registrierung.getVollstaendigerImpfschutzTyp() != null
			&& registrierung.getVollstaendigerImpfschutzTyp().isExternesZertifikat()) {
			return false;
		}
		...
		if (impfung.getTermin().getImpffolge() == Impffolge.ERSTE_IMPFUNG) {
			...
			if (registrierung.isGenesen()) {
				if (ImpfinformationenUtil.isRelevantImpfungBeforeCoronaTest(registrierung,
	...
	public static boolean deservesAndWantsZertifikat(
		...
		if (!deservesZertifikat(registrierung, impfung, externesZertifikat)) {
			return false;
		}
