...
public class ImpfinformationenUtil {
	...
	public static boolean isRelevantImpfungBeforeCoronaTest(@NonNull Registrierung registrierung, @NonNull LocalDateTime relevantImpfungTimestamp) {
		boolean vacmeGenesen =
			registrierung.getVollstaendigerImpfschutzTyp() == VollstaendigerImpfschutzTyp.VOLLSTAENDIG_VACME_GENESEN;
		if (vacmeGenesen) {
			...
			final LocalDate pcrDatum = registrierung.getPositivGetestetDatum();
			if (pcrDatum != null) {
	...
	public static boolean willBeGrundimmunisiertAfterErstimpfungImpfstoff(
		...
		if (externesZertifikat != null
			&& externesZertifikat.getMissingForGrundimmunisiert() == MissingForGrundimmunisiert.BRAUCHT_1_IMPFUNG) {
			return true;
		}
