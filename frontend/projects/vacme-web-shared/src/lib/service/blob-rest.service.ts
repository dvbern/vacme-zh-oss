...
import {FileInfoJaxTS} from 'vacme-web-generated';
...
export class BlobRestService {
    ...
    public postFiles$(filesToUpl: File[], registrierungsnummer: string | undefined): Observable<Array<FileInfoJaxTS>> {
        const formData = new FormData();
        for (const file of filesToUpl) {
            formData.append('file', file, encodeURIComponent(file.name));
        }
        return this.postRequest$(registrierungsnummer, formData);
    }

    private postRequest$(registrierungsnummer: string | undefined, formData: FormData): Observable<any> {
        const url = `${this.serviceURL}/registrierungsnummer/${registrierungsnummer}`;
