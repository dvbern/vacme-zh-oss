...
export class BerufUtil {
    ...
    public static addBerufAutoselectForChildren(formGroup: FormGroup, geburtstagKey: string, berufKey: string): void {
        formGroup.get(geburtstagKey)?.valueChanges.subscribe(() => {
            this.onGeburtsdatumChange(formGroup, geburtstagKey, berufKey);
        }, err => LOG.error(err));
    }

    private static onGeburtsdatumChange(formGroup: FormGroup,
    ...
    private static isTooYoungForWork(formGroup: FormGroup, geburtstagKey: string): boolean {
