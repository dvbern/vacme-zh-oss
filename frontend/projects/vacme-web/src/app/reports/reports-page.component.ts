...
export class ReportsPageComponent extends BaseDestroyableComponent implements OnInit {
    ...
    constructor(
        ...
        private reportsSyncService: ReportsSyncService,
    ) {
        super();
        const minLength = 2;
    ...
    ngOnInit(): void {
        ...
        if (this.authService.hasRole(TSRole.OI_IMPFVERANTWORTUNG)) {
            this.ortderimpfungService.ortDerImpfungResourceGetAllOrtDerImpfungJax().pipe().subscribe(
    ...
    public downloadReportingKantonEnabled(): boolean {
    ...
    public downloadAbrechnungErwachsen(): void {
        ...
        FormUtil.doIfValid(this.formGroup, () => {
            ...
            this.reportsService.reportResourceGenerateExcelReportAbrechnungErwachsenAsync(this.translationService.currentLang,
                ...
                dateVon) // asynchron
                .subscribe(() => {
                    this.triggeredReprotSubject$.next(this.translationService.instant('REPORTS.ABRECHNUNG_ERWACHSEN.FILE_NAME'));
                    Swal.hideLoading();
                    this.reportInProgress = false;
                }, error => {
                    Swal.hideLoading();
    ...
    public downloadAbrechnungKind(): void {
        ...
        FormUtil.doIfValid(this.formGroup, () => {
            ...
            this.reportsService.reportResourceGenerateExcelReportAbrechnungKindAsync(this.translationService.currentLang,
                ...
                dateVon) // asynchron
                .subscribe(() => {
                    this.triggeredReprotSubject$.next(this.translationService.instant('REPORTS.ABRECHNUNG_KIND.FILE_NAME'));
                    Swal.hideLoading();
                    this.reportInProgress = false;
                }, error => {
                    Swal.hideLoading();
    ...
    public downloadAbrechnungZHEnabled(): boolean {
    ...
    public downloadImpungenFHIR(): void {
        ...
        FormUtil.doIfValid(this.formGroupFhir, () => {
            ...
            this.downloadService.downloadResourceDownloadFhirImpfdokumentation(code)
                .subscribe(
                    (res) => {
                        this.filesaver.save(res, this.translationService.instant('REPORTS.IMPFUNGEN_FHIR.FILE_NAME', {regNum: code}));
                        Swal.hideLoading();
                        this.reportInProgress = false;
                    }, error => {
                        Swal.hideLoading();
