...
export class ImpfdokumentationGrundimunisierungPageComponent extends BaseDestroyableComponent implements OnInit {
    ...
    private confirmDosisCountAndSave(impfdokumentation: ImpfdokumentationJaxTS,
                                     wasImpfungNachtragen: undefined | boolean,
                                     saveFunction: (impfdokumentation: ImpfdokumentationJaxTS,
    ...
    private onAfterSaved(
        ...
        if (wasImpfungNachtragen === true
            ...
            this.router.navigate(['dossier', impfdokumentation.registrierungsnummer]);
        } else {
            this.print();
    ...
    public isUserBerechtigtForOdiOfTermin(): boolean {
        switch (this.impffolge) {
            case ImpffolgeTS.ERSTE_IMPFUNG:
                return this.isUserBerechtigtForOdiOfTermin1or2(this.dashboardJax?.termin1);
            case ImpffolgeTS.ZWEITE_IMPFUNG:
                return this.isUserBerechtigtForOdiOfTermin1or2(this.dashboardJax?.termin2);
            case ImpffolgeTS.BOOSTER_IMPFUNG:
                throw Error('falsche impffolge in impfdokuemntation grundimmunisierung ' + this.impffolge);
        }
    }

    private isUserBerechtigtForOdiOfTermin1or2(termin?: ImpfterminJaxTS): boolean {
        if (!termin) {
            return true;
        }
    ...
    private reloadPage(modifFlag: boolean, erstTerminAdHocFlag: boolean,
                       anchor: 'terminbuchung-anchor' | 'buttons-anchor' | 'termine-anchor'): void {
        this.router.navigate(
            [],
            {
    ...
    public gotoNextFreienTermin($event: any): void {
        this.terminmanager.gotoNextFreienTermin('impfdokumentation', $event);
    }
