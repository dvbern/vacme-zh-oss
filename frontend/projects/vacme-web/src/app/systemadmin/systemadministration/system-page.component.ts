...
export class SystemPageComponent extends BaseDestroyableComponent implements OnInit {
    ...
    public callPdfArchivierung(): void {
        ...
        this.actionInProgress = true;
        this.developService.developerResourceCallPdfArchivierung()
            .subscribe(res => {
                Swal.hideLoading();
                this.actionInProgress = false;
            }, error => {
                Swal.hideLoading();
                this.actionInProgress = false;
                LOG.error('PDF Archivierung konnten nicht gestartet werden', error);
            });
    }

    public callDeactivateUnusedUsers() {
        if (this.actionInProgress) {
            return;
        }
        this.actionInProgress = true;
    ...
    public callEnsureImpdossierExistsService() {
