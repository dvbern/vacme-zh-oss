...
import {NextFreierTerminSearch,} from '../../../../vacme-web-shared/src/lib/components/vacme-components/termine-bearbeiten/termine-bearbeiten.component';
...
export class TerminmanagementKontrolleImpfdokService {
    ...
    constructor(private terminfindungService: TerminfindungService,
    ...
    public umbuchungRequestPut$(impffolge: ImpffolgeTS, registrierungsNummer: string): Observable<DashboardJaxTS> {
    ...
    public reinitializeFromDashboard(dashboardJax: DashboardJaxTS, forceReload: boolean,
                                     ...
                                     kontrolleOrImpfdok: { isUserBerechtigtForOdiOfTermin: () => boolean }): void {
        const dossierChange = this.hasChangedDossiernummer(dashboardJax);
    ...
    private resetSlotsInTerminfindungIfNichtBerechtigt(impffolge: ImpffolgeTS | ImpffolgeTS.ZWEITE_IMPFUNG | ImpffolgeTS.BOOSTER_IMPFUNG) {
    ...
    private hasChangedDossiernummer(dashboardJax: DashboardJaxTS): boolean {
    ...
    private setOdiInTerminfindung(dashboardJax: DashboardJaxTS, impffolge: ImpffolgeTS): void {
        ...
        this.terminfindungService.ortDerImpfung = dashboardTermin?.impfslot?.ortDerImpfung || dashboardJax.gewuenschterOrtDerImpfung;
    }
    ...
    public gotoNextFreienTermin(referrer: string, $event: NextFreierTerminSearch): void {
        this.terminbuchungService
            .terminbuchungResourceGetNextFreierImpftermin(
                $event.impffolge, $event.ortDerImpfungId, $event.nextDateParam)
            .subscribe(
                nextReturned => {
                    if (nextReturned) { // Termin gefunden -> navigieren
                        ...
                        if ($event.otherTerminDate) { // Warscheinlich ist die Regel +-28 Tage nicht erfuellt
                            ...
                            const errMsg = this.translationService.instant('ERROR_NO_TERMINE_IN_RANGE', {desiredDistance: distText});
    ...
    public identifyOdiIdToSelect(odiList: OrtDerImpfungJaxTS[]): string | null {
        if (odiList.length === 1) { // wenn nur 1 direkt selecten
            ...
            return this.odiIdIfActiveOrNull(odiId, odiList);
        } else {
    ...
    private odiIdIfActiveOrNull(odiId: string | null, odiList: OrtDerImpfungJaxTS[]): string | null {
        if (odiId) {
