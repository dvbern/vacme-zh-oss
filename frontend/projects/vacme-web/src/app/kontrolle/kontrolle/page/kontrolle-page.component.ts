...
import {
    ...
    PrioritaetTS, RegistrierungsEingangTS,
...
export class KontrollePageComponent extends BaseDestroyableComponent implements OnInit {
    ...
    private loadDashboard$(regNummer: string | undefined): Observable<DashboardJaxTS | undefined> {
        if (regNummer) {
            return this.dossierService.dossierResourceGetDashboardRegistrierung(regNummer);
        } else {
            return of(undefined);
        }
    }
    ...
    private onChangedParams(
        ...
        if (this.canShowValidierungen()) {
            this.registrierungValidationService.validateTermineAndStatusAndUserodi(
                this.impfkontrolle, this.impffolge, this.ortDerImpfungList,
            );
        }
    ...
    public onFormLoaded(): void {
        ...
        if (this.impfkontrolle) {
            this.kontrolleForm.patchValue(this.impfkontrolle, this.dashboardJax?.impfdossier);
            this.kontrolleForm.disableFields();
    ...
    title(): string {
        ...
        if (!!this.newPerson) {
            return this.translationService.instant('FACH-APP.KONTROLLE.TITLE-NEW');
        }

        return this.translationService.instant('FACH-APP.KONTROLLE.TITLE',
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            {i: this.impfkontrolle!.impffolgeNr});
    ...
    public showExternGeimpft(): boolean {
        ...
        const hasVacmeImpfung = BoosterUtil.hasVacmeImpfung(dashboard);
    ...
    print(): void {
        ...
        this.dossierService.dossierResourceDownloadRegistrierungsbestaetigung(
            this.regNummer).subscribe(res => {
    ...
    public gotoNextFreienTermin($event: NextFreierTerminSearch): void {
        this.terminmanager.gotoNextFreienTermin('kontrolle', $event);
    }
    ...
    public navigateToImpfdokumentation(): void {
        if (this.impffolge === ImpffolgeTS.BOOSTER_IMPFUNG) {
            this.router.navigate(['person', this.regNummer, 'impfdokumentation', 'booster']);
        } else {
            this.router.navigate(['person', this.regNummer, 'impfdokumentation']);
        }
    }
