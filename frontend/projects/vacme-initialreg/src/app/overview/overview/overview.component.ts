...
import {MAX_ODI_DISTANCE, WARN_ALTER_IMPFLING} from '../../../../../vacme-web-shared/src/lib/constants';
...
export class OverviewComponent implements OnInit, OnChanges, OnDestroy {
    ...
    public createOdiOptions(): Array<IOdiOption> {
        ...
        if (!this.isImmobil() || this.stationaryImpfort) {
            options.push({
    ...
    public showSaveButtonForOdiWithTermin(): boolean {
        return this.isFreigegebenOderMehr()
            ...
            && !this.terminUtilService.hasOdiNoFreieTermineForMe(this.dashboardJax, this.getOrtDerImpfung());
    ...
    public ortWaehlen(): void {
        ...
        FormUtil.doIfValid(this.erkrankungenConfirmFromGroup, () => {
            ...
            if (this.nichtVerwalteterOdiSelected) {
                this.dossierService.dossierResourceRegSelectNichtVerwalteterOrtDerImpfung(registrierungsnummer)
                    .subscribe(
                        () => {
                ...
                this.dossierService.dossierResourceRegSelectOrtDerImpfung(selectedOrt).subscribe(
                    () => {
    ...
    public impfdokuEnabled(): boolean {
        ...
        return BoosterUtil.hasVacmeImpfung(this.dashboardJax);
    }
    ...
    public canEditExternesZertifikat(): boolean {
        ...
        if (BoosterUtil.hasVacmeImpfung(this.dashboardJax)) {
            return false;
        }
    ...
    public resendTerminbestaetigung(): void {
        ...
        this.dossierService.dossierResourceRegTerminbestaetigungErneutSenden(registrierungsnummer)
            .subscribe(
                (res: RegistrierungsCodeJaxTS) =>
                    Swal.fire({
                        icon: 'success',
    ...
    public getBoosterFreigabetext(): string {
        ...
        if (RegistrierungStatusTS.FREIGEGEBEN_BOOSTER === status) {
            return this.translationService.instant('OVERVIEW.FREIGEGEBEN_BOOSTER');
        }

        if (RegistrierungStatusTS.IMMUNISIERT === status) {
            // nicht freigegeben, und nicht selbstzahler angeklickt
            if (!this.vacmeSettingsService.boosterFreigabeNotificationDisabled && !this.isSelbstzahler()) {
                return this.translationService.instant('OVERVIEW.FREIGEGEBEN_BOOSTER_NICHT', {
    ...
    isTerminReadOnly(impffolge: ImpffolgeTS): boolean {
        if (impffolge === ImpffolgeTS.ERSTE_IMPFUNG || impffolge === ImpffolgeTS.BOOSTER_IMPFUNG) {
            ...
            return this.isAtLeastOdiGewaehltButNotYetGeimpftValues() || this.terminfindungService.selectedSlot1 === undefined;
        }
    }
    ...
    private absagen(): void {
        ...
        if (registrierungsNummer) {
            this.dossierService.dossierResourceRegOdiAndTermineAbsagen(registrierungsNummer)
                .subscribe(res => {
    ...
    public showHintNichtVerwalteterOdi(): boolean {
        return this.isNichtVerwalteterOdi()
            && (!isAnyStatusOfCurrentlyAbgeschlossen(this.terminfindungService.dashboard.status) || this.isSelbstzahler());
    }
    ...
    public getLastErkrankungDate(): string | undefined {
        if (this.dashboardJax?.impfdossier?.erkrankungen?.length) {
            const theDate = (this.dashboardJax.impfdossier!.erkrankungen![this.dashboardJax.impfdossier!.erkrankungen!.length - 1])!.date;
    ...
    canResendOnboardingLetter(): boolean {
        ...
        return onboardingEnabled
            ...
            && isAtLeastOnceGeimpft(this.dashboardJax.status);
    }
    ...
    resendOnboardingLetter(): void {
        ...
        if (regNr) {
            Swal.fire({
                icon: 'question',
                text: this.translationService.instant('OVERVIEW.ONBOARDING_LETTER.CONFIRM.TEXT'),
                confirmButtonText: this.translationService.instant('OVERVIEW.ONBOARDING_LETTER.CONFIRM.CONFIRM'),
                showCancelButton: true,
                cancelButtonText: this.translationService.instant('OVERVIEW.ONBOARDING_LETTER.CONFIRM.CANCEL'),
            }).then(r => {
                if (r.isConfirmed) {
                    this.onboardingService.onboardingResourceTriggerOnboardingLetter(regNr)
                        .subscribe(() => {
                            Swal.fire({
                                icon: 'success',
                                text: this.translationService.instant('OVERVIEW.ONBOARDING_LETTER.BESTAETIGUNG'),
                                timer: 2000,
                                showCloseButton: false,
                                showConfirmButton: false,
                            });
