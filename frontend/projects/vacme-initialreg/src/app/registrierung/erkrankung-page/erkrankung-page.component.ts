...
import {DashboardJaxTS, DossierService, ErkrankungDatumHerkunftTS, ErkrankungJaxTS} from 'vacme-web-generated';
...
export class ErkrankungPageComponent extends BaseDestroyableComponent implements OnInit, CanComponentDeactivateSimple {
        ...
        this.route.data
            ...
            .subscribe(next => {
                ...
                this.formGroup = this.fb.group({
                    erkrankungen: this.fb.array(erkrankungen.map((erkrankung, index) => this.createErkrankungFormgroup(
                        erkrankung,
                        index))),
                });
     ...
     * fields
     */
    private addValueChangeListenersForDependentValidation(erkFormGrp: FormGroup) {
    ...
    private prepareLoadToSave(): { data: ErkrankungJaxTS[] | undefined; regNr: string } {
        const data = this.getErkrankungenFormArray()?.controls
            .filter(control => control.get('erkrankungdatumHerkunft')?.value === ErkrankungDatumHerkunftTS.ERFASST_IMPFLING)
    ...
    public reValidateAllControlsExceptPassed(erkFormGrp: FormGroup) {
